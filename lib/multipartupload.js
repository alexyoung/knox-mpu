...
function MultiPartUpload(opts, callback) {
    ...
    this.partSize = opts.partSize || 1242880; // 5MB default
    this.maxRetries = opts.maxRetries || 4;   // default to no retry
    this.uploadId = null;
    ...
    if (opts.stream) {
      console.log('[knox-mpu] opts._putStream');
    ...
    (function ensureTmpDirExists(afterwards){
      ...
      mpu.on('partProgress', function(data) {
          console.log('[knox-mpu] partProgress:', data.written);
      ...
      if (opts.stream) {
        console.log('[knox-mpu] opts._putFile');
    ...
    fs.exists(file, function(exists) {
        ...
        fs.lstat(file, function (err, stats) {
            ...
            while (remainingBytes > mpu.partSize) {
                ...
                parts.push(part);
                console.log('[knox-mpu] _putFile mpu._uploadPart.bind');
            ...
            mpu._initiate(function(err, uploadId) {
                if (err || !uploadId) {
                    console.log('[knox-mpu] _putFile _initiate error:', err || uploadId);
                    return callback('Unable to initiate file upload');
                }
                console.log('[knox-mpu] _putFile _initiate callback');
...
MultiPartUpload.prototype._putStream = function(stream, callback) {
    ...
    console.log('[knox-mpu] _putStream');
    var mpu = this;
...
MultiPartUpload.prototype._handleStream = function(stream, callback) {
    ...
    function newPart() {
        ...
        parts.push(part);
        console.log('[knox-mpu] newPart done:', partFile);
        return part;
    }
    ...
    stream.on('data', function(buffer) {
        ...
        if((mpu.currentUploadSize += buffer.length )> mpu.maxUploadSize){
            console.log('[knox-mpu] abortUpload');
    ...
    stream.on('end', function() {
        if (current) {
            console.log('[knox-mpu] partReady');
            partReady(current);
        }

        // Wait for the completion of the uploads
        if(mpu.aborted){
          console.log('[knox-mpu] mpu._abortUploads');
          return mpu._abortUploads(callback);
        }else{
          console.log('[knox-mpu] mpu._completeUploads');
          return mpu._completeUploads(callback);
        }
    });
...
MultiPartUpload.prototype._uploadPart = function(part, callback) {
    ...
    console.log('[knox-mpu] _uploadPart:', arguments);
    console.trace();

    // If we haven't started the upload yet, wait for the initialization
    if (!this.uploadId) {
        console.log('[knox-mpu] _uploadPart: this.on initiated');
        return this.on('initiated', this._uploadPart.bind(this, part, callback));
    }

    console.log('[knox-mpu] _uploadPart: setting up request');
    ...
    console.log('[knox-mpu] _uploadPart: PUT request being made to', url, headers);

    // Wait for the upload to complete
    req.on('response', function(res) {
        console.log('[knox-mpu] _uploadPart: response');
        ...
        console.log('[knox-mpu] _uploadPart: response statusCode', res.statusCode);
        ...
        if (res.statusCode != 200  && !part.alreadyCB) {
            ...
            console.log('[knox-mpu] _uploadPart failed');
            // If we haven't used up all of our retries, attempt to upload again
            if (--part.triesLeft) {
                console.log('[knox-mpu] _uploadPart: response retry');
                return MultiPartUpload.prototype._uploadPart.call(mpu, part, callback);
            }
            // Otherwise bail out
            else {
                console.log('[knox-mpu] _uploadPart: response bail');
                part.alreadyCB = true;
        ...
        console.log('[knox-mpu] _uploadPart: uploaded');

        mpu.emit('uploaded', result);

        // If we were saving to a temp file, delete it now
        if (!this.noDisk) {
            console.log('[knox-mpu] _uploadPart: no disk, unlink');
            fs.unlink(part.fileName, function(err) {
                // Make sure the callback is only called once
                console.log('[knox-mpu] _uploadPart: call callback 0');
                if (!part.alreadyCB){
                    console.log('[knox-mpu] _uploadPart: call callback 1');
                    part.alreadyCB = true;
                    return callback(err, result);
                }
            });
        } else if (!part.alreadyCB) {
            console.log('[knox-mpu] _uploadPart: call callback 2');
            // Make sure the callback is only called once
            part.alreadyCB = true;
    ...
    if (!this.noDisk) {
        ...
        console.log('[knox-mpu] _uploadPart partStream pipe');
        partStream.pipe(req);
    } else {
        console.log('[knox-mpu] _uploadPart req.write');
        req.write(part.data);
        req.end();
    }

    console.log('[knox-mpu] _uploadPart uploading...');
...
MultiPartUpload.prototype._completeUploads = function(callback) {
    ...
    console.log('[knox-mpu] Entering _completeUploads');

    var mpu = this;

    this.uploads.drain = function() {
        console.log('[knox-mpu] _completeUploads: drain');
        ...
        parse.xmlResponse(req, function(err, body) {
            console.log('[knox-mpu] _completeUploads: parse xmlResponse started');
            if (err) return callback(err);
            if (body === null) return callback(new Error('got empty response'));
            delete body.$;
            body.size = mpu.size;
            mpu.emit('completed', body);
            console.log('[knox-mpu] _completeUploads: parse xmlResponse completed');
        ...
        console.log('[knox-mpu] _completeUploads: req.end()');
    };
};
